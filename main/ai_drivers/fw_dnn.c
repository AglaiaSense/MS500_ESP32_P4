...
static const char *CONFIG_FILE_NETWK_CSTM = "/spiffs/dnn/network_info.txt";
...
#if 0
        printf("[DNN_LOADER] dnnType=%d \n",s_dnn_nw_type);
        printf("[DNN_LOADER] networkID=%06X \n",s_dnn_nw_id);
        printf("[DNN_LOADER] dnnHeaderSize=%d \n",s_nw_info_list[k].dnnHeaderSize);
        printf("[DNN_LOADER] inputTensorWidth=%d \n",s_nw_info_list[k].inputTensorWidth);
        printf("[DNN_LOADER] inputTensorHeight=%d \n",s_nw_info_list[k].inputTensorHeight);
        printf("[DNN_LOADER] inputTensorWidthStride=%d \n",s_nw_info_list[k].inputTensorWidthStride);
        printf("[DNN_LOADER] inputTensorHeightStride=%d \n",s_nw_info_list[k].inputTensorHeightStride);
        printf("[DNN_LOADER] inputTensorSize=%d \n",s_nw_info_list[k].inputTensorSize);
        printf("[DNN_LOADER] inputTensorFormat=%d \n",s_nw_info_list[k].inputTensorFormat);
        printf("[DNN_LOADER] inputTensorNorm_K00=0x%04X \n", s_nw_info_list[k].NormK00);
        printf("[DNN_LOADER] inputTensorNorm_K02=0x%04X \n", s_nw_info_list[k].NormK02);
        printf("[DNN_LOADER] inputTensorNorm_K03=0x%04X \n", s_nw_info_list[k].NormK03);
        printf("[DNN_LOADER] inputTensorNorm_K11=0x%04X \n", s_nw_info_list[k].NormK11);
        printf("[DNN_LOADER] inputTensorNorm_K13=0x%04X \n", s_nw_info_list[k].NormK13);
        printf("[DNN_LOADER] inputTensorNorm_K20=0x%04X \n", s_nw_info_list[k].NormK20);
        printf("[DNN_LOADER] inputTensorNorm_K22=0x%04X \n", s_nw_info_list[k].NormK22);
        printf("[DNN_LOADER] inputTensorNorm_K23=0x%04X \n", s_nw_info_list[k].NormK23);
        printf("[DNN_LOADER] yClip=0x%08X \n", s_nw_info_list[k].yClip);
        printf("[DNN_LOADER] cbClip=0x%08X \n", s_nw_info_list[k].cbClip);
        printf("[DNN_LOADER] crClip=0x%08X \n", s_nw_info_list[k].crClip);
        for (size_t j = 0; j < BAYER_CH_MAX; j++) {
            printf("[DNN_LOADER] inputTensorRGBNorm_CH%d=0x%04X \n", j, s_nw_info_list[k].rgbNorm[j].add);
            printf("[DNN_LOADER] inputTensorRGBNormShift_CH%d=0x%02X \n", j, s_nw_info_list[k].rgbNorm[j].shift);
            printf("[DNN_LOADER] inputTensorRGBNormCripMax_CH%d=0x%04X \n", j, s_nw_info_list[k].rgbNorm[j].clipMax);
            printf("[DNN_LOADER] inputTensorRGBNormCripMin_CH%d=0x%04X \n", j, s_nw_info_list[k].rgbNorm[j].clipMin);
        }
        printf("[DNN_LOADER] inputTensorNorm_YGain=0x%02X \n", s_nw_info_list[k].yGgain);
        printf("[DNN_LOADER] inputTensorNorm_YAdd=0x%03X \n", s_nw_info_list[k].yAdd);
        printf("[DNN_LOADER] outputTensorNum=%d \n",s_nw_info_list[k].outputTensorNum);
        for (size_t j = 0; j < s_nw_info_list[k].outputTensorNum; j++) {
            sc_output_tensor_size_info_t* p_info = &s_nw_info_list[k].p_outputTensorSizeInfo[j];
            p_info->tensorSize = (p_info->dimSize + p_info->padding) * p_info->bytePerElement;
            printf("[DNN_LOADER] [nw_info[%d]] outputTensorSize%d=%d \n", k, j, p_info->tensorSize);
        }
#endif
    }
