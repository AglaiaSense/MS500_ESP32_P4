...
static int parseOutputTensorBody(const uint8_t* src, const uint16_t lineSize, FrameOutputInfo* frameOutputTensorInfo) {
	...
	uint8_t* tmp_dst_buf = (uint8_t *)malloc(frameOutputTensorInfo->outputBodyInfo->totalSize * sizeof(float));
	if (tmp_dst_buf == NULL) {
	...
	for (unsigned int i = 0; i < idxs.size(); i++) {
		...
		threads[tensorIdx] = std::move(std::thread([&](int tensorIdx, const uint8_t* src, int offset) -> int {		
			size_t outputTensorSize = outSizes[tensorIdx];
			uint16_t numOfLines = numOfLinesVec[tensorIdx];
			bool sortingRequired = false;

			OutputTensorApParams param = frameOutputTensorInfo->outputApParams->at(tensorIdx);
			std::vector<Dimensions> serialized_dim = serialized_dims[tensorIdx];// (param.numOfDimensions);
			std::vector<Dimensions> actual_dim = actual_dims[tensorIdx]; //(param.numOfDimensions);
			...
			if (param.bitsPerElement == 8) {
				for (int i = 0; i < numOfLines; i++) {
					...
					while (lineIndex < frameOutputTensorInfo->dnnHeaderInfo->maxLineLen) {
						if (param.format == TYPE_SIGNED) {
							int8_t temp = (int8_t)*(src + lineIndex);
							float value = (temp - param.shift) * param.scale;
							tmp_dst[offset + elementIndex] = value;
						} else {
							uint8_t temp = (uint8_t)*(src + lineIndex);
							float value = (temp - param.shift) * param.scale;
							tmp_dst[offset + elementIndex] = value;
						}
						elementIndex++;
				...
				for (int i = 0; i < numOfLines; i++) {
					...
					while (lineIndex < frameOutputTensorInfo->dnnHeaderInfo->maxLineLen) {
						if (param.format == TYPE_SIGNED) {
							int16_t temp = bytes_to_int16((int8_t)*(src + lineIndex + 1), (int8_t)*(src + lineIndex));	// TODO
							float value = (temp - param.shift) * param.scale;
							tmp_dst[offset + elementIndex] = value;
						} else {
							uint16_t temp = bytes_to_uint16((uint8_t)*(src + lineIndex + 1), (uint8_t)*(src + lineIndex));	// TODO
							float value = (temp - param.shift) * param.scale;
							tmp_dst[offset + elementIndex] = value;
						}
						elementIndex++;
			...
			if (sortingRequired) {
				...
				if (param.bitsPerElement == 8) {
					memcpy(dst + offset, tmp_dst + offset, outputTensorSize * sizeof(float));
				} else if (param.bitsPerElement == 16) {
					memcpy(dst + offset, tmp_dst + offset, (outputTensorSize >> 1) * sizeof(float));
				} else {
					printf("Invalid bitsPerElement value =%d", param.bitsPerElement);
					retStatus = -1;
					free(tmp_dst_buf);
			...
			return 0;
		}, tensorIdx, src_arr[tensorIdx], offsets[tensorIdx]));
	}
