...
extern char car_reader[12];
extern char car_mutex;
extern char lp_detected;
...
static int decode_yolo(const void *data) {
    ...
    std::vector<std::array<int, 3>> sizes = {{1, 4, 3549}, {1, 1, 3549}}; // 416x416
...
int decode_plate(const void *buf) {
    ...
    int pre_str = 0;
    for (int i = 0; i < 21; i++) {
        if (array_index[i] != 0 && array_index[i] != pre_str) {
    ...
    for (int i = 0; i < digits; i++) {
        ...
        // 第6次检查到车牌再输出
        if(cnt == 4 ) {
            if (plate_str[i] >= 1 && plate_str[i] <= 41) {
                int index = 1 + (plate_str[i] - 1) * 3;
                car_reader[j++] = plate_chr[index];
                car_reader[j++] = plate_chr[index+1];
                car_reader[j++] = plate_chr[index+2];
                printf("%c%c%c", plate_chr[index], plate_chr[index + 1], plate_chr[index + 2]);
            }
            else if (plate_str[i] == 77 || plate_str[i] == 78) {
                int index = 158 + plate_str[i] - 77;
                car_reader[j++] = plate_chr[index];
                car_reader[j++] = plate_chr[index+1];
                car_reader[j++] = plate_chr[index+2];
                printf("%c%c%c", plate_chr[index], plate_chr[index + 1], plate_chr[index + 2]);
    ...
    if (cnt == 4) {
        memset(car_reader, '0', sizeof(car_reader));
        // 将整数转换为字符并赋值给字符数组的第一个元素
        car_reader[0] = '0' + digits;
        ...
        for (int i = 0; i < digits; i++) {
            ...
            if (plate_str[i] >= 1 && plate_str[i] <= 34) {
                ...
                car_reader[i + 1] = plate_chr[index];
                printf("%c", plate_chr[index]);
            }
        }
        printf("\r\n");
        ...
        // 进入light sleep 模式
        // printf("Enter into light sleep mode...\n");
